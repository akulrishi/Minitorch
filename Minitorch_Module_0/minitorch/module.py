class Module:
    ...
    def train(self):
        ...
        self.training = True
        for i, j in self.__dict__["_modules"].items():
            self.__dict__["_modules"][i].training = True
    ...
    def eval(self):
        ...
        self.training = False
        for i, j in self.__dict__["_modules"].items():
            self.__dict__["_modules"][i].training = False
        # raise NotImplementedError('Need to implement for Task 0.4')
    ...
    def parameters(self):
        ...
        #         name,node = queue_elem
        #         if name != "":
        #             name = name + "."
        #         for i, j in node._parameters.items():
        #             names.append((name + i, j))
        #         for i, j in node._modules.items():
        #             queue.append((name + i, j))

        def dfs_param_names(node,):
            visited = {}
            stack = [("", node)]

            while (len(stack)):
                stack_elem = stack.pop()
                name, node = stack_elem
                if (node not in visited):
                    ...
                    for i, j in node._parameters.items():
                        names.append((name + i, j))
                    for i, j in node._modules.items():
                        stack.append((name + i, j))
        dfs_param_names(self)
        return names
